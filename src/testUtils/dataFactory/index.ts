...
const getRepositoryValues = ({ ownerName, repoName }: GetRepositoryValues) => {
  const repository =
    repoName ??
    faker
...
type RepositoryEventMap = {
  archived: EmitterWebhookEvent<'repository.archived'>;
  created: EmitterWebhookEvent<'repository.created'>;
};

type BuildRepositoryEventProps<TAction extends RepostioryEventActions> = {
  action: TAction;
  isFork?: boolean;
  ownerName?: string;
  repoName?: string;
...
export const buildRepositoryEvent = <TAction extends RepostioryEventActions>({
  action,
  isFork,
  ownerName,
  repoName,
}: BuildRepositoryEventProps<TAction>): RepositoryEventMap[TAction] => {
  const {
    repository,
    repositoryId,
    owner,
    ...
    sender,
    senderId,
  } = getRepositoryValues({ ownerName, repoName });

  return {
    id: faker.random.uuid(),
    name: 'repository' as 'repository',
    payload: {
      action,
  ...
  } as RepositoryEventMap[TAction];
};

type BuildRepositoryPayloadProps = {
  isFork: boolean;
  repository: string;
  repositoryId: number;
...
const buildRepositoryPayload = ({
  ...
  owner,
  ownerId,
}: BuildRepositoryPayloadProps): Repository => {
  return {
    id: repositoryId,
...
export const buildPushEvent = ({
  ...
  repoName,
  commits,
}: BuildPushEventProps = {}): EmitterWebhookEvent<'push'> => {
  ...
  const pushData: EmitterWebhookEvent<'push'> = {
    id: faker.random.uuid(),
    name: 'push',
    payload: {
      ref: branchRef,
